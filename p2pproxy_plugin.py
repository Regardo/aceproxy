...
class P2pproxy(object):
    handlers = ('channels', 'channels.m3u', 'archive', 'xbmc.pvr', 'logobase', 'ace')
    ...
    def handle(self, connection):
        ...
        elif connection.reqtype == 'ace':
            P2pproxy.logger.warning('Get ace')
            P2pproxy.logger.warning(self.params.get('infohash')[0])
            P2pproxy.logger.warning(connection.headers['Host'])
            P2pproxy.logger.warning('http://' + connection.headers['Host'] + '/infohash/' + self.params.get('infohash')[0] + '/stream.mp4')
            connection.send_response(301)
            connection.send_header('Location', 'http://' + connection.headers['Host'] + '/infohash/' + self.params.get('infohash')[0] + '/stream.mp4')
            connection.end_headers()
